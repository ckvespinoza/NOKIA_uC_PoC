...
int main(void)
{
  ...
  osThreadDef(NormalStage, NormalStage_init, osPriorityNormal, 0, 128);
  ...
  osThreadDef(MonHResetReq, MonHResetReq_init, osPriorityAboveNormal, 0, 128);
  MonHResetReqHandle = osThreadCreate(osThread(MonHResetReq), NULL);

  /* definition and creation of MonSResetReq */
  osThreadDef(MonSResetReq, MonSResetReq_init, osPriorityBelowNormal, 0, 128);
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(GPIOB, LD1_Pin|TEST_EN_Pin|LD3_Pin|LD2_Pin, GPIO_PIN_RESET);
...
void NormalStage_init(void const * argument)
{
  ...
  {
	printf("Normal Stage\n");
	HAL_GPIO_WritePin(TEST_EN_GPIO_Port, TEST_EN_Pin, SET);
    osDelay(1000);
  }
  /* USER CODE END 5 */
}
...
void MonHResetReq_init(void const * argument)
{
  ...
  for(;;)
  {
    printf("Polling for H_RESET_REQ...\n");
	if(HAL_GPIO_ReadPin(H_RESET_REQ_GPIO_Port, H_RESET_REQ_Pin) == true)
	{
	  printf("Hard Reset Request received!\n");
	}
    osDelay(1000);
  }
...
void MonSResetReq_init(void const * argument)
{
  ...
  for(;;)
  {
	 printf("Polling for S_RESET_REQ...\n");
	if(HAL_GPIO_ReadPin(S_RESET_REQ_GPIO_Port, S_RESET_REQ_Pin) == true)
	{
	  printf("Soft Reset Request received!\n");
	}
    osDelay(1000);
  }
